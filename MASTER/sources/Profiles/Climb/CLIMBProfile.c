...
bStatus_t ClimbProfile_AddService(uint32 services) {
	...
	if (services & CLIMBPROFILE_SERVICE) {
		...
		status = GATTServApp_RegisterService(climbProfileAttrTbl, GATT_NUM_ATTRS(climbProfileAttrTbl),
		GATT_MAX_ENCRYPT_KEY_SIZE, &climbProfileCBs);
	} else {
		status = SUCCESS;
	}
...
//correct way, the len parameter is not used, the notification will be MAX_MTU_SIZE - 3
			...
			ret = GATTServApp_ProcessCharCfg(climbProfileChar1Config, climbProfileChar1, len, FALSE, climbProfileAttrTbl, GATT_NUM_ATTRS(climbProfileAttrTbl),
